...
# Crear el marco para el conversor de temperatura
converter_frame = tk.Frame(notebook, bg="#202020")
notebook.add(converter_frame, text="Conversor de Temperatura")
...
# Crear el conversor de temperatura
def convert_temperature():
    try:
        temp = float(temp_entry.get())
        if temp_unit.get() == "Celsius a Fahrenheit":
            result = (temp * 9/5) + 32
            result_label.config(text=f"Resultado: {result:.2f} °F", fg="white")
        elif temp_unit.get() == "Fahrenheit a Celsius":
            result = (temp - 32) * 5/9
            result_label.config(text=f"Resultado: {result:.2f} °C", fg="white")
        else:
            result_label.config(text="Unidad no válida", fg="#d32f2f")
    except ValueError:
        result_label.config(text="Entrada inválida", fg="#d32f2f")

tk.Label(converter_frame, text="Conversor de Temperatura", font=("Segoe UI", 16), bg="#202020", fg="white").pack(pady=10)

temp_entry = tk.Entry(converter_frame, font=("Segoe UI", 14), justify="center", bg="#2d2d2d", fg="white")
temp_entry.pack(pady=10)

temp_unit = ttk.Combobox(converter_frame, values=["Celsius a Fahrenheit", "Fahrenheit a Celsius"], font=("Segoe UI", 12))
temp_unit.pack(pady=10)
temp_unit.current(0)

tk.Button(converter_frame, text="Convertir", font=("Segoe UI", 14), bg="#1a73e8", fg="white", command=convert_temperature).pack(pady=10)

result_label = tk.Label(converter_frame, text="", font=("Segoe UI", 14), bg="#202020", fg="white")
result_label.pack(pady=10)

# Ajustar el tamaño de las columnas y filas
for i in range(8):
    root.grid_rowconfigure(i, weight=1)
for j in range(5):  # Incrementar el rango para incluir la columna del historial
